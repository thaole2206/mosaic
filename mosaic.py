...
def timeit(func):
    ...
    def decorate(*args, **kwargs):
        ...
        print("function %s cost: %s" % (func.__name__, diff))
        return result
...
class TileProcessor:
    ...
    def __process_tile(self, tile_path):
        try:
            ...
            return (large_tile_img.convert(COLOR_MODE), small_tile_img.convert(COLOR_MODE))
        except Exception as e:
            print("process tile error: ", e)
            return (None, None)
    ...
    def get_tiles(self):
        ...
        for root, subFolders, files in os.walk(self.tiles_directory):
            ...
            for tile_name in files:
                ...
                mirror_and_rotate_image = [tile, tile.transpose(Image.FLIP_LEFT_RIGHT), tile.transpose(Image.FLIP_TOP_BOTTOM),
                                           tile.rotate(90), tile.rotate(180), tile.rotate(270)]
                for tile_s in mirror_and_rotate_image:
                    large_tile, small_tile = self.__process_tile(tile_s)
                    if large_tile:
...
class TargetImage:
    ...
    def get_data(self):
        ...
        image_data = (large_img.convert(COLOR_MODE), small_img.convert(COLOR_MODE))
...
def fit_tiles(work_queue, result_queue, tiles_data):
    ...
    while True:
        try:
            ...
            while best_fit == False:
                tile_index = tile_fitter.get_best_fit_tile(img_data)
                if tile_index not in best_tile_index_list:
                    best_tile_index_list.append(tile_index)
                    result_queue.put((img_coords, tile_index))
                    break

                if fail_counter == TILE_REUSE:
                    best_tile_index_list = []
                    fail_counter = 0
...
class MosaicImage:
    ...
    def add_tile(self, tile_data, coords):
        img = Image.new(COLOR_MODE, (TILE_SIZE, TILE_SIZE))
...
def compose(original_img, tiles):
    ...
    try:
        ...
        print('Halting, saving partial image please wait...')
...
if __name__ == '__main__':
    ...
    # mosaic("target3LA.png", "testSource")